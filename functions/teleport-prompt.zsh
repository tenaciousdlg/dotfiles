...
#
#   ⛨ blackhat ▸ chris 7h32m           (green: >4h left on the TLS cert)
#   ⛨ blackhat ▸ bob ↑staging 5h02m    (an approved access request is assumed)
#   ⛨ blackhat ▸ bob 6h02m             (magenta: persona shell via TELEPORT_HOME)
#   ⛨ blackhat ▸ chris 43m             (yellow <1h, red <15m / expired)
...
#
# Enable by adding `teleport` to POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS.
# Override colors with POWERLEVEL9K_TELEPORT_<STATE>_FOREGROUND
...
_teleport_cert_meta() {
  ...
  if [[ -z ${_tp_expiry_memo[$key]} ]]; then
    ...
    [[ -n $end ]] || return 1
    exp=$(date -j -u -f '%b %e %H:%M:%S %Y %Z' "$end" +%s 2>/dev/null) || return 1
...
prompt_teleport() {
  ...
  if [[ -z $prof ]]; then
    p10k segment -s LOGGEDOUT -f 8 -i $'' -t 'logged out'
    return
  fi
  ...
  local user=${certs[1]:t:r}         # chris.delagarza@goteleport.com
  user=${user%%@*}                   # chris.delagarza
  user=${user%%.*}                   # chris
  local cluster=${prof%%.*}          # blackhat
  ...
  p10k segment -s "${persona}${state}" -f $fg -i $'' -t "${cluster} ▸ ${user}${elev} ${ttl}"
}
